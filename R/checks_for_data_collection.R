...
# Logical checks ----------------------------------------------------------
# okay_parents_arrange_child_marriage_not_agree
...
# okay_parents_arrange_child_marriage_agree
df_c_logic_okay_parents_arrange_child_marriage_agree <- df_tool_data %>% 
  ...
  mutate(i.check.type = "change_response",
         ...
         i.check.current_value = okay_parents_arrange_child_marriage,
         i.check.value = "",
         i.check.issue_id = "okay_parents_arrange_child_marriage_agree",
         i.check.issue = glue("okay_parents_arrange_child_marriage_for_money: {okay_parents_arrange_child_marriage_for_money},  
                              okay_parents_arrange_child_marriage_for_her_safety: {okay_parents_arrange_child_marriage_for_her_safety}"),
         i.check.other_text = "",
         i.check.checked_by = "",
...
girl_less_18_years_get_married_not_agree <- df_tool_data %>% 
  filter(okay_girl_less_18_years_get_married %in% c("disagree", "strongly_disagree") & 
           okay_girl_stay_home_and_stop_school_once_married %in% c("agree", "strongly_agree")) %>% 
  mutate(i.check.type = "change_response",
         i.check.name = "okay_girl_less_18_years_get_married",
         i.check.current_value = okay_girl_less_18_years_get_married,
         i.check.value = "",
         i.check.issue_id = "okay_girl_less_18_years_get_married_not_agree",
...
girl_less_18_years_get_married_not_agree_b <- girl_less_18_years_get_married_not_agree %>% 
...
df_c_logic_okay_girl_less_18_years_get_married_not_agree <- bind_rows(girl_less_18_years_get_married_not_agree, girl_less_18_years_get_married_not_agree_b) %>% 
  dplyr::select(starts_with("i.check"))%>% 
  rename_with(~str_replace(string = .x, pattern = "i.check.", replacement = ""))

if(exists("df_c_logic_okay_girl_less_18_years_get_married_not_agree")){
  if(nrow(df_c_logic_okay_girl_less_18_years_get_married_not_agree) > 0){
    logic_output$df_c_logic_okay_girl_less_18_years_get_married_not_agree <- df_c_logic_okay_girl_less_18_years_get_married_not_agree
  }
}
