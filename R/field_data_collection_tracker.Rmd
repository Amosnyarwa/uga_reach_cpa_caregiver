---
title: "Caregiver: Data collection Tracker"
...
output: 
  html_document:
    ...
    toc_float:
      collapsed: false
      smooth_scroll: false
knit: (function(inputFile, encoding) { rmarkdown::render(inputFile, encoding = encoding, output_file = paste0(dirname(inputFile), '/caregiver_field_data_collection_tracker_', format(Sys.Date(), '%Y_%m_%d'),'.html')) })
---
...
# read data 
df_settlement_samples_required <- readr::read_csv("../inputs/Refugee_caregiver_survey_samples_definition.csv") %>% 
...
df_tool_data <- readxl::read_excel("../inputs/UGA2109_Cross_Sectoral_Child_Protection_Assessment_Caregiver_Data.xlsx") %>% 
  ...
  filter(consent_two == "yes",  respondent_age >= 12,
...
df_cl_log <- read_csv(file = "../inputs/combined_checks_caregiver.csv") %>% 
...
df_updated_tool_data %>% 
  ...
  left_join(df_refugee_samp_per_settlement, by = "refugee_settlement") %>% 
...
df_updated_tool_data %>% 
  ...
  summarise(number_of_surveys = n()) %>% 
  arrange(district_name) %>% 
  left_join(df_host_samp_per_sub_county, by = "sub_county_div") %>% 
